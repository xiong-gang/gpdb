...
class SQLIsolationExecutor(object):
    ...
    class SQLConnection(object):
        def __init__(self, out_file, name, utility_mode, dbname, port=None):
            self.name = name
        ...
        def session_process(self, pipe):
            sp = SQLIsolationExecutor.SQLSessionProcess(self.name, 
                self.utility_mode, pipe, self.dbname, self.port)
            sp.do()
    ...
    class SQLSessionProcess(object):
        def __init__(self, name, utility_mode, pipe, dbname, port=None):
            """
                Constructor
            """
            self.name = name
            ...
            if self.utility_mode:
                ...
                self.con = self.connectdb(given_dbname=self.dbname, given_port=port)
    ...
    def get_process(self, out_file, name, utility_mode=False, dbname="", port=None):
        """
        ...
        if not (name, utility_mode) in self.processes or port is not None:
            if not dbname:
                dbname = self.dbname
