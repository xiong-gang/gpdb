...
static bool compareCaps(const ResGroupCaps *caps1, const ResGroupCaps *caps2);
...
static bool
compareCaps(const ResGroupCaps *caps1, const ResGroupCaps *caps2)
{
	const ResGroupCap	*p1 = (ResGroupCap *) caps1;
	const ResGroupCap	*p2 = (ResGroupCap *) caps2;
	int i;

	for (i = 0; i < RESGROUP_LIMIT_TYPE_COUNT; i++)
		if (p1[i].value != p2[i].value || p1[i].proposed != p2[i].proposed)
			return false;
